...
int main(int argc, char *argv[])
{
	...
	if (!getparint("useOuterreflectivity", &useOuterreflectivity))
		useOuterreflectivity = 0;
...
if(useOuterreflectivity==0)
{
	rfp = fopen(reftfile, "wb+");
}
else
{
	rfp = fopen(reftfile, "rb+");
}
...
else
{
	for (ix = 0; ix < nx; ix++)
	{
		fread(refl[ix], sizeof(float), nz, rfp);
	}
	fclose(rfp);
}
#endif
	...
	for (ishot = 0; ishot < nshot; ishot = ishot + 1)
	{
		...
		totaltr = headInformation[ishot].totaltr;
