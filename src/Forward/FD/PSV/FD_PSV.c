...
void FD_PSV()
{
	...
	NDT sample for the forward modeled wavefield is written to su files*/
        ...
        /* create model grids */
        readmod_elastic_PSV(matPSV.prho, matPSV.ppi, matPSV.pu, matPSV.eta_x, matPSV.eta_z, matPSV.lambda);
        ...
        av_mue(matPSV.pu, matPSV.puipjp, matPSV.prho, matPSV.ppi);

        av_rho(matPSV.prho, matPSV.prip, matPSV.prjp);

        if ((FREE_SURF) && (POS[2] == 0) && (!TOPO))
        {

                // Jian Cao: Adaptive free-surface expression for flat interface
                for (i=1; i<=NX; i++)
                {
                matPSV.prip[1][i]=2.*matPSV.prip[1][i];
                matPSV.eta_x[1][i]=(matPSV.eta_x[1][i] - matPSV.lambda[1][i]) * ((matPSV.eta_x[1][i] - matPSV.lambda[1][i])/2. + matPSV.lambda[1][i]) / matPSV.eta_x[1][i];
                matPSV.eta_z[1][i]=0.;
                matPSV.lambda[1][i]=0.;
                }
		...
		==================================================================================*/
                ...
                psv(&wavePSV, &wavePSV_PML, &matPSV, &mpiPSV, &seisPSV, &acq, hc, ishot, nshots, nsrc_loc, ns, ntr, Ws, Wr, req_send, req_rec);
