	  ...
	  /* bottom boundary */                                         
        if((POS[2]==NPROCY-1) && (j>=ny2-FW+1)){
			...
			//       syy[j][i] += g*(vxx+vyy)+(2.0*f*vyy);
                              sxx[j][i] += eta_x[j][i] * vxx + lambda[j][i] * vyy;
			      syy[j][i] += eta_z[j][i] * vyy + lambda[j][i] * vxx;
			}
		}
		break;
		...
		for (j=ny1;j<=ny2;j++){
			for (i=nx1;i<=nx2;i++){
				...
				// syy[j][i]+=((g*(vxx+vyy))-(2.0*f*vxx))*dhi;
                                ...
                                sxx[j][i] += (eta_x[j][i] * vxx + lambda[j][i] * vyy) *dhi;
			        syy[j][i] += (eta_z[j][i] * vyy + lambda[j][i] * vxx) *dhi;
			}
		}
		break;
