...
void psv(struct wavePSV *wavePSV, struct wavePSV_PML *wavePSV_PML, struct matPSV *matPSV, struct mpiPSV *mpiPSV,
		 ...
		 int ns, int ntr, float **Ws, float **Wr, MPI_Request *req_send, MPI_Request *req_rec)
{
	...
	for (nt = 1; nt <= NT; nt++)
	{
		...
		update_s_elastic_PML_PSV(1, NX, 1, NY, (*wavePSV).pvx, (*wavePSV).pvy, (*wavePSV).ux, (*wavePSV).uy, (*wavePSV).uxy, (*wavePSV).uyx, (*wavePSV).psxx, (*wavePSV).psyy, (*wavePSV).psxy, (*matPSV).eta_x, (*matPSV).eta_z,
									(*matPSV).puipjp, (*wavePSV_PML).absorb_coeff, (*matPSV).lambda, hc, infoout, (*wavePSV_PML).K_x, (*wavePSV_PML).a_x, (*wavePSV_PML).b_x, (*wavePSV_PML).K_x_half, (*wavePSV_PML).a_x_half,
