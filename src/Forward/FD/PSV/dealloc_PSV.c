...
void dealloc_PSV(struct wavePSV *wavePSV, struct wavePSV_PML *wavePSV_PML){
        ...
        /* global variables */
	extern int NX, NY, L, FW, FDORDER, FRE_SNAP;
	...
	if(FRE_SNAP){
		...
		free_matrix((*wavePSV).vx_real,1,NY,1,NX);
		free_matrix((*wavePSV).vx_imag,1,NY,1,NX);
		free_matrix((*wavePSV).vy_real,1,NY,1,NX);
		free_matrix((*wavePSV).vy_imag,1,NY,1,NX);
	}
