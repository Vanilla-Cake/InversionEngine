...
void scanimg(int type, float spx, float bcx, float dx, float dz, Cell ***c1, Cell ***c2, int live, int dead, int nt, float dt, float ft,
			 ...
			 float **g, float fpx, float dpx, float **imb, float **ampgrid,float **tang);
...
void csmiggb(int type, float bwh, float fmin, float fmax, float amin, float amax, int live,
			 ...
			 float **f, float **v, float **g, float *coordx,float **tang);
...
int main(int argc, char *argv[])
{
	...
	for (ishot = 0 + myid; ishot < nshot; ishot = ishot + np)
	{
		...
		coord = (float *)malloc(sizeof(float) * ntr);
		for(i=0;i<ntr;i++) {
			coord[i]=firp*dx+i*dx;
		}
		...
		csmiggb(MIG, bwh, fmin, fmax, amin, amax, live, dead, nt, dt, sp*dx, firp*dx, lastp*dx,
						apernum, dx, ntr, dtr, nz, dz, orif, tempv, tempg, coord, tang);
...
*****************************************************************************/
{
	...
	for (ixb = 0, xb = fxb + ixb * dxb; ixb < nxb; ++ixb, xb += dxb)
	{
		...
		if ((rayxp >= 0) && (rayxp < nx) && (xb >= fx) && (xb < (fx + (nx - 1) * dx)))
		{
			...
			scanimg(type, spx, xb, dx, dz, cell1, cell2, live, dead, ntau, dtau, ftau, fmin, lx, lz, nx, nz, mx, mz, npx, bm[ixb], g, fpx, dpx, imb, ampgrid,tang);
...
static void cellBeam(int type, float spx, float bcx, float dx, float dz, Cell **cell, float **f, float **g, int jx, int jz, BeamData *bd,
					 int lx, int lz, int nx, int nz, int npx, int ips, int ipr, float fpx, float dpx, 
					float ft, float dt, int nt, float **imb,float **tang);
			...
			for (imx = 0; imx < mx - 1; imx++)
			{
				for (imz = 0; imz < mz - 1; imz++)
				{
						...
						(fncell[imx][imz + 1].live == live) && (fncell[imx + 1][imz + 1].live == live) && (abs(fncell[imx + 1][imz + 1].ip - fncell[imx][imz + 1].ip) < 2) && (abs(fncell[imx][imz + 1].ip - fncell[imx][imz].ip) < 2) && (abs(fncell[imx][imz].ip - fncell[imx + 1][imz].ip) < 2) && (abs(fncell[imx + 1][imz].ip - fncell[imx + 1][imz + 1].ip) < 2))
					{
						...
						cellBeam(type, spx, bcx, dx, dz, fncell, f, g, imx, imz, bd, lx, lz, nx, nz, npx, ips, ipr, fpx, dpx, ft, dt, nt, imb,tang);
					} // end if
