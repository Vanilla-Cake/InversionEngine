...
/* one step conjugate-direction descent method */
/* if set iter = 1, then it is basiclly steep descent*/
...
int main(int argc, char *argv[])
{
	...
	FILE *vfp;
	FILE *sfp,*dfp,*rfp,*ufp,*mfp;		/* temp file to hold traces	*/
	...
	seifile="temp.su";
	imgfile="lsm-mig-marfd.dat";
	reftfile="reflectivity.dat";
	orgfile="shot.su";
	orimigfile="ini-mig.dat";

	nshot=240;      /*total shot number*/
	ntr=96;        /*trace number*/
	...
	/* open files */
	vfp=fopen(vfile,"rb+");
//	sfp=fopen(seifile,"wb+");
	dfp=fopen(imgfile,"wb+");
	rfp=fopen(reftfile,"wb+");
	ufp=fopen(orgfile,"rb+");
	mfp=fopen(orimigfile,"wb+");
	...
	lsmg = alloc2float(nz,nx);
	vsm = alloc2float(nz,nx);
	residual = alloc1double(nter+10);
	oriv = alloc2float(nz,nx);
	origall = alloc2float(nz,nx);
	lsmgall = alloc2float(nz,nx);
...
/*	fclose(sfp);*/
	...
	smooth2d(nz, nx, 7.0, 7.0, vsm);


	/* calculate the reflectivity by loca velocity perturbation */
	for (ix=0; ix<nx; ix++) {
		for (iz=0; iz<nz; iz++) {
			v[ix][iz]=2*(v[ix][iz]-vsm[ix][iz])/vsm[ix][iz]/v[ix][iz]/v[ix][iz];
			v[ix][iz]*=1000*1000*1000;
		}
	}

#if 1 //write out the reflectivity
	for(ix=0;ix<nx;ix++)
	{
  	      fwrite(v[ix],sizeof(float),nz,rfp);
	}
	fclose(rfp);
#endif
	...
	memset(&lsmg[0][0],0,sizeof(float)*nx*nz);
	memset(&residual[0],0,sizeof(double)*(nter+1));
	memset(&origall[0][0],0,sizeof(float)*nx*nz);
	memset(&lsmgall[0][0],0,sizeof(float)*nx*nz);


	/* First step, calculate modeled data and migrate */
        for(ishot=1+myid; ishot<=nshot; ishot=ishot+np) 
	{
		inputrace(ishot,nt,dx,maxtrace,ufp,&sp,&firp,&ntr);
		sp=sp-cdpmin;
		firp=firp-cdpmin;
		...
		memset(&orif[0][0],0,sizeof(float)*ntr*nt);
    		...
    		/* forward model, v is reflectivity here */
		//csmiggb(MOD,bwh,fmin,fmax,amin,amax,live,dead,nt,dt,sp,firp,
		//	apernum,dx,ntr,dtr,nz,dz,orif,tempv,v);

		for(i=0;i<ntr;i++) {
			fseek(ufp,60*4,1);
			fread(orif[i],sizeof(float),nt,ufp);
			kt=NINT(450 - i*4.2);
			for(j=0;j<kt;j++)
				orif[i][j] = 0.0;		
		}
					
		/* alloc space for temparray */
		f=alloc2float(nt,ntr);
		tempf=alloc2float(nt,ntr);
		templsmg = alloc2float(nz,apernum);
		s = alloc2float(nz,apernum);
		ss = alloc2float(nt,ntr);

 	  	/* forward model, v is reflectivity here */
		memset(&f[0][0],0,sizeof(float)*ntr*nt);
		memset(&templsmg[0][0],0,sizeof(float)*nx*apernum);
		...
		if(ishot==1) {
			for(i=0;i<ntr;i++)
				for(j=0;j<nt;j++)
					residual[1] += f[i][j]*f[i][j];
			residual[1]=sqrt(residual[1]);
		}
		
		/* iterate with shots */
		for(iter=1; iter<=nter; iter++) 
		{	
			printf("SECOND STEP: Shot number is %d; iteration number is %d \n",ishot,iter);								
			...
			csmiggb(MIG,bwh,fmin,fmax,amin,amax,live,dead,nt,dt,sp,firp,
				apernum,dx,ntr,dtr,nz,dz,f,tempv,tempg);

			/* if iter==1, write out the initial image */
			if(iter==1) {
				/* accmulate the subimage to allimage */
				partall(-1,nz,apernum,apermin,tempg,orig);
			}
#if 1	
    			/* forward model the residual image*/
			memset(&tempf[0][0],0,sizeof(float)*ntr*nt);
			csmiggb(MOD,bwh,fmin,fmax,amin,amax,live,dead,nt,dt,sp,firp,
				apernum,dx,ntr,dtr,nz,dz,tempf,tempv,tempg);
		
			/* update g and f using conjugate direction method */
			cgstep(iter, templsmg, tempg, f, tempf, apernum, nz, ntr, nt, s, ss);
	
			/* only calculate residual of one shot */
			if(ishot==1) {
				for(i=0;i<ntr;i++)
					for(j=0;j<nt;j++)
						residual[iter+1] += f[i][j]*f[i][j];
				residual[iter+1]=sqrt(residual[iter+1]);
			}
#endif
		...
		/* accmulate the subimage to allimage */
		partall(-1,nz,apernum,apermin,templsmg,lsmg);

		if(ishot==1) { 
			for(iter=1;iter<=nter+1;iter++) 
				printf(" residual of %d is %lf\n", iter, residual[iter]);
		}
        ...
        MPI_Reduce(&lsmg[0][0],&lsmgall[0][0],nx*nz,MPI_FLOAT,MPI_SUM,0,MPI_COMM_WORLD);
	...
	if(myid==0) {
		...
		for(ix=0;ix<nx;ix++)
		{
			fwrite(origall[ix],sizeof(float),nz,mfp);
		}
		for(ix=0;ix<nx;ix++)
		{
			fwrite(lsmgall[ix],sizeof(float),nz,dfp);
		}
	}	
...
//	fprintf(stderr,"npx=%d dpx=%g fpx=%g\n",npx,dpx,fpx);
	...
	for (ipx=0; ipx<npx; ipx++) {
		px=fpx+ipx*dpx;
		pz=dpx/sqrt(1.0/(vmin*vmin)-px*px);
	...
	for(imx=0;imx<mx;imx++) {
		for(imz=0;imz<mz;imz++)
		{
			...
			if(ampgrid[imx][imz]>ampmax) ampmax=ampgrid[imx][imz];	
		}
	}
	...
	for(imx=0;imx<mx;imx++) {
		for(imz=0;imz<mz;imz++)
		{
			ampgrid[imx][imz] = ampgrid[imx][imz]+ampmax/200;
			ampgrid[imx][imz] = 1.0;
		}
	}
...
//			fprintf(stderr,"ixb/nxb = %d/%d  ix = %d ipx=%d ntau=%d\n",ixb,nxb,ixlo,npx,ntau);
			...
			/* main function of model or migration using beams */
		        scanimg(type, cell1, cell2, live, dead, ntau, dtau, ftau,fmin, lx, lz, nx, nz, mx, mz, npx, b, g, fpx, dpx, imb, ampgrid);
...
void cgstep(int iter, float **x, float **g, float **rr, float **gg, int nx, int nz, int ntr, int nt, float **s, float **ss)
{
	...
	if(iter==1) {
		...
		if(dotgg == 0.0) printf("Error happens beacause grad vanishes identically");
		...
		if((gdg==0.0)||(sds==0.0)) printf("gdg = 0.0, or sds = 0.0\n");
		...
		for(ipr=0;ipr<npx;ipr++)
		{
			...
			for(imx=0;imx<mx;imx++)
			{
				...
				//	  printf("%d %d %f\n",ipr, tp, tempt );
					...
					if(type == -1) fncell[imx][imz].ai = fncell[imx][imz].ai/sqrt(ampgrid[imx][imz]), fncell[imx][imz].ar = fncell[imx][imz].ar/sqrt(ampgrid[imx][imz]);
					...
					//printf("%d %d %d %d %d\n",c2[ipr][imx][imz].ip,ipr,fncell[imx][imz].ip,fncell[imx][imz].live,live);
...
//			printf("%f %f %f %d %d %f\n",taxzr,taxzi,dtaxzr,cell[jx][jz].ip,ipr,tin);
			...
			if(fabs(azg)>80*PI/180) continue;
