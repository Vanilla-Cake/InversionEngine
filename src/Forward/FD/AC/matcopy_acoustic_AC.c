...
void matcopy_acoustic_AC(float ** rho, float ** pi){
	...
	extern int MYID, NX, NY, INDEX[5], POS[3], NPROCX, NPROCY;
...
/*	if (POS[2]!=NPROCY-1)*/	/* no boundary exchange at bottom of global grid */
	...
	MPI_Bsend(&buffertop_to_bot_1[0][1],(NX+2)*2,MPI_FLOAT,INDEX[3],TAG5,SHOT_COMM);
	MPI_Barrier(SHOT_COMM);
	MPI_Recv(&buffertop_to_bot_1[0][1],(NX+2)*2,MPI_FLOAT,INDEX[4],TAG5,SHOT_COMM,&status);
	MPI_Bsend(&bufferbot_to_top_1[0][1],(NX+2)*2,MPI_FLOAT,INDEX[4],TAG6,SHOT_COMM);
	MPI_Barrier(SHOT_COMM);
	MPI_Recv(&bufferbot_to_top_1[0][1],(NX+2)*2,MPI_FLOAT,INDEX[3],TAG6,SHOT_COMM,&status);   
...
/*	if (POS[1]!=NPROCX-1)*/	/* no boundary exchange at right edge of global grid */
	for (j=0;j<=NY+1;j++){
 	...
 	MPI_Bsend(&bufferlef_to_rig_1[0][1],(NY+2)*2,MPI_FLOAT,INDEX[1],TAG1,SHOT_COMM);
	MPI_Barrier(SHOT_COMM);
	MPI_Recv(&bufferlef_to_rig_1[0][1],(NY+2)*2,MPI_FLOAT,INDEX[2],TAG1,SHOT_COMM,&status);
	MPI_Bsend(&bufferrig_to_lef_1[0][1],(NY+2)*2,MPI_FLOAT,INDEX[2],TAG2,SHOT_COMM);
	MPI_Barrier(SHOT_COMM);
	MPI_Recv(&bufferrig_to_lef_1[0][1],(NY+2)*2,MPI_FLOAT,INDEX[1],TAG2,SHOT_COMM,&status);
